  ...
  return (
    <div className="md:hidden">
      <button
        ...
        className="flex items-center justify-center rounded-md border border-line p-2 text-ink"
      >
      ...
      {open && (
        <div className="absolute inset-x-0 top-full border-b border-line bg-white">
            ...
            {[{ href: "/", label: "Главная" }, ...NAV_ITEMS, { href: "/search", label: "Поиск" }].map(
              (item) => (
                <Link
                  ...
                  className="border-b border-line py-3 text-sm font-medium text-ink last:border-b-0"
                >
                  {item.label}
                </Link>
              ),
